...
class MADDPG:
    def __init__(
        ...
        for obs_dim, act_dim in zip(obs_dim_list, act_dim_list):
            self.agents.append(
                Agent(
                    obs_dim, act_dim, global_obs_dim, actor_lr, critic_lr, self.device
                )
            )
    ...
    def select_action(self, obs):
        actions = []
        for n, agent in enumerate(
            self.agents
        ):  # each agent select action according to their obs
            o = (
                torch.from_numpy(obs[n]).unsqueeze(0).float().to(self.device)
            )  # torch.Size([1, state_size])
    ...
    def learn(self, batch_size, gamma):
        for i, agent in enumerate(self.agents):
            obs, act, reward_cur, next_obs, done_cur, next_act, obs_cur = self.sample(
                batch_size, i
            )
            # update critic
            ...
            self.logger.info(
                f"agent{i}: critic loss: {critic_loss.item()}, actor loss: {actor_loss.item()}"
            )
