...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--episode-length", type=int, default=25, help="steps per episode"
    )
    parser.add_argument(
        "--episode-num", type=int, default=30000, help="total number of episode"
    )
    parser.add_argument("--gamma", type=float, default=0.95, help="discount factor")
    parser.add_argument("--buffer-capacity", default=int(1e6))
    parser.add_argument("--batch-size", default=1024)
    parser.add_argument(
        "--actor-lr", type=float, default=1e-2, help="learning rate of actor"
    )
    parser.add_argument(
        "--critic-lr", type=float, default=1e-2, help="learning rate of critic"
    )
    parser.add_argument(
        "--steps-before-learn",
        type=int,
    ...
    for episode in range(args.episode_num):
        ...
        print(
            f"episode {episode + 1}: cumulative reward: {cumulative_reward}, "
    ...
    print(
        f"training finishes, time spent: {datetime.timedelta(seconds=int(time() - start))}"
    )
